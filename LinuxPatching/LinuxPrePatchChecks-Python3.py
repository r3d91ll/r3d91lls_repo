...
class PrePatchCheck:
    ...
    def check_python_version():
        pass
    python3_available = sys.version_info >= (3, 0)
    if python3_available:
        print("Python 3 is available. Running Python 3 script...")
        # Run the Python 3 script
        subprocess.run(["python3", "your_python3_script.py"])
    else:
        python2_available = sys.version_info >= (2, 7) and sys.version_info < (3, 0)
        if python2_available:
            print("Python 2.7 is available. Downloading and running Python 2.7 script...")
            # Download the Python 2.7 script
            script_url = "https://inc0361448.s3.us-gov-west-1.amazonaws.com/LinuxPrePatchChecks-Python2.7.py"
            try:
                urllib.request.urlretrieve(script_url, "LinuxPrePatchChecks-Python2.7.py")
                print("Python 2.7 script downloaded successfully.")
            except Exception as e:
                print(f"Failed to download Python 2.7 script: {e}")
                sys.exit(1)

            # Run the Python 2.7 script
            subprocess.run(["python2", "LinuxPrePatchChecks-Python2.7.py"])
        else:
            print("No compatible Python version found. Please install Python 2.7 or Python 3.")
            sys.exit(1)
    print("Python 2.7 is available. Downloading and running Python 2.7 script...")
    # Download the Python 2.7 script
    script_url = "https://inc0361448.s3.us-gov-west-1.amazonaws.com/LinuxPrePatchChecks-Python2.7.py"
