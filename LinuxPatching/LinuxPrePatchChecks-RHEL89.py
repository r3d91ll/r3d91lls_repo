...
class PrePatchCheck:
    ...
    def get_crowdstrikeVersion(self):
        ...
        # Try to get the CrowdStrike version
        try:
            ...
            self.log("Error fetching CrowdStrike version. Checking if the service is running.")
            ...
            if "inactive" in service_status.stdout:
                ...
                subprocess.run(["systemctl", "start", "falcon-sensor"])
                time.sleep(2)  # Wait for 2 seconds

                # Try to get the CrowdStrike version again after starting the service
                try:
                    result = subprocess.run(["/opt/CrowdStrike/falconctl", "-g", "--version"], capture_output=True, text=True)
                    crowdstrike_version = result.stdout.strip()
                    ...
                    self.log("Error fetching CrowdStrike version even after starting the service.")
                    self.update_prepatch_report("Manual remediation - Crowdstrike Error")
                    return "CrowdStrike error"
            else:
                self.log("CrowdStrike service is running but version fetch failed. Restarting the service.")
                subprocess.run(["systemctl", "restart", "falcon-sensor"])
                time.sleep(5)  # Wait for 5 seconds

                # Try to get the CrowdStrike version again after restarting the service
                try:
                    result = subprocess.run(["/opt/CrowdStrike/falconctl", "-g", "--version"], capture_output=True, text=True)
                    crowdstrike_version = result.stdout.strip()
                    self.log("CrowdStrike version after service restart: {}".format(crowdstrike_version))
                    ...
                    self.log("Error fetching CrowdStrike version even after restarting the service.")
                    self.update_prepatch_report("Manual remediation - Crowdstrike Error")
                    return "CrowdStrike error"
